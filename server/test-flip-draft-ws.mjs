...
process.env.DRAFT_PICK_TIMEOUT_MS = '5000';
...
const room = roomManager.createRoom(
  ...
  true,
  'flip ws',
  { draftMode: 'flip', flipTargetCards: 1, flipMarketRowSize: 2 },
);
...
await alice.waitFor('draft_started');
await bob.waitFor('draft_started');

let aliceState = await alice.waitFor('flip_state');
assert.equal(aliceState.payload.isYourTurn, true);
let card = pickAffordable(aliceState.payload, 4);
alice.send('flip_buy_card', { roomId: room.id, marketSlot: card.marketSlot, cardId: card.id });

let bobState = await bob.waitFor('flip_state', 1000, msg => msg.payload?.isYourTurn === true);
assert.equal(bobState.payload.activePlayerName, 'Bob');
card = pickAffordable(bobState.payload, 4);
bob.send('flip_buy_card', { roomId: room.id, marketSlot: card.marketSlot, cardId: card.id });

const complete = await alice.waitFor('draft_complete', 1500);
assert.equal(complete.payload.mode, 'flip');
assert.equal(complete.payload.pools[alice.playerId].cardIds.length, 1);
assert.equal(complete.payload.pools[bob.playerId].cardIds.length, 1);
...
console.log('[test-flip-draft-ws] ok');
process.exit(0);
...
async function openClient(url) {
  ...
  const client = {
    ...
    waitFor(type, timeoutMs = 1000, predicate = null) {
      const existing = this.messages.find(msg => msg.type === type && (!predicate || predicate(msg)));
...
function pickAffordable(state, funds) {
  ...
  assert.ok(card, 'expected an affordable public card');
  return card;
}

...
当前 testMode 已不再用固定调试卡自动补足。快速提交会先调用 `/api/cards/script-status` 过滤缺脚本卡；如果用户手动提交的 YDK 仍然缺脚本，服务端会在注册 neos 房间前报清楚错误，不再等到 neos 里显示“版本不匹配”。

### 建议

- 记录哪些卡常缺
- 判断这些缺失是环境问题、数据问题，还是测试牌组问题
- 继续统计哪些 cube 卡经常缺脚本，必要时在 cube 管理或轮抽结果页提前提示
